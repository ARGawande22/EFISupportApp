...
namespace EFISupportApp
{
    ...
    public class ReadPaySlipPDF
    {
        ...
        private static void ParseTable(List<PdfLine> block, Employee emp)
        {
            ...
            // 8 anchor X-positions from the header row:
            // [EmolPart, EmolAmt, GovtPart, GovtAmt, GovtInst, NGPart, NGAmt, NGInst]
            var anchors = BuildColumnAnchors(headerWords);
            if (anchors.Count < 8) return; // header didn't parse as expected - bail safely

            // IMPORTANT: we deliberately do NOT split "Amount" from "Inst. No."
            // by X-position. Amount values are right-aligned in a narrow
            // column, so a plain amount with no instalment reference next to
            // it (e.g. "I.Tax 9000") drifts across whatever boundary we'd
            // draw and lands in the Inst.No band purely by chance of
            // horizontal position. Instead we treat Amount+Inst.No as ONE
            // combined region per side-table and separate them afterward by
            // *pattern* (an instalment reference always looks like "11/36"
            // or "8/10" - anything else in that region is the amount).
            //
            // So we only need boundaries between:
            //   EmolPart | EmolAmt | GovtPart | GovtValue | NGPart | NGValue
            // -> 5 boundaries, 6 bands.
            var boundaries = new List<double>
            {
                (anchors[0] + anchors[1]) / 2.0, // EmolPart | EmolAmt
                (anchors[1] + anchors[2]) / 2.0, // EmolAmt  | GovtPart
                (anchors[2] + anchors[3]) / 2.0, // GovtPart | GovtValue
                (anchors[4] + anchors[5]) / 2.0, // GovtValue| NGPart
                (anchors[5] + anchors[6]) / 2.0, // NGPart   | NGValue
            };

            // Rows to scan: everything strictly between header and totals line.
            // Re-flatten to raw words and re-cluster ourselves (rather than
            // trusting the pre-existing PdfLine grouping) - a row that mixes
            // text and numbers of different glyph heights can otherwise get
            // split across two "lines", which desyncs Particular from its
            // Amount. See ClusterWordsIntoLines for the full rationale.

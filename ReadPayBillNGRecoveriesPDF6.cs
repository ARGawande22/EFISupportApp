...
{
    public class ReadPayBillNGRecoveriesPDF6
    {
        /// <summary>
        ...
        private static readonly Regex RowPattern = new Regex(
                                            ...
                                            @"(?<numbers>(?:[\d,]+\s+){1,}[\d,]+)\s*" +
                                            @"(?:\((?<code2>[A-Z0-9]+)\)\s*)?" +
                                            @"\(\s*(?<desig>(?:[A-Za-z]+|\([A-Za-z\s]+\))(?:\s+(?:[A-Za-z]+|\([A-Za-z\s]+\)))*)\s*\)",
        ...
        private static DataTable BuildEmployeeTable(string text, List<WordInfo> words, bool dumpDebugText)
        {
            ...
            foreach (var m in matches)
            {
                ...
                string code = m.Groups["code0"].Success
                                ? m.Groups["code0"].Value.Trim()
                                : (m.Groups["code1"].Success
